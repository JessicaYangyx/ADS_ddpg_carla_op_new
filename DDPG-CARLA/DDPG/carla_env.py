...
reward_mode = 2  #0: speed reward | 1: anangle reward with Speed = 1 | 2: angle reward as a function of speed
...
class CarEnv:
    ...
    tf_session2 = tf.compat.v1.Session(config=config2)

    tf.compat.v1.keras.backend.set_session(tf_session2)
    ...
    def reset(self):
        ...
        transform_for_obs_sensor = carla.Transform(carla.Location(x=2.5, z=0.7))
        obstaclesensor = self.blueprint_library.find("sensor.other.obstacle")
        self.obstaclesensor = self.world.spawn_actor(obstaclesensor, transform_for_obs_sensor, attach_to=self.vehicle)
        ...
        self.obstaclesensor.listen(lambda event4: self.obstacle_data(event4))
        ...
        #self.state_train = self.Calculate_state(self.front_camera)
    ...
    def obstacle_data(self, event4):
        self.front_obs_dist = event4.distance
        self.front_actor = event4.other_actor
    ...
    def step(self, action):
        ...
        # OUTPUT USED FOR THE PROGRAM OF WAYPOINTS OBTAINED BY IMAGE PROCESSING
        if settings.WORKING_MODE == settings.WORKING_MODE_OPTIONS[1]:
            if done == True:
                self.distance_acum.append(acum)
            state = self.Calculate_state(self.front_camera)
        ...
        # OUTPUT USED BY THE REST OF THE PROGRAMS
            ...
            # IF YOU HAVE TURNED AND NO WAYPOINT IS SEEN AHEAD, JUST GO AWAY.
            if exit_flag == 1:
                print('Waypoints have been lost, distance to target: ', d2target)
                done = True
    ...
    def get_reward(self):
        ...
        exit_program = 0  # PROGRAM EXIT CONDITION.

        d2target = self.distance_target(self.Target, location)
        # EXIT CONDITION IF THERE IS A COLLISION
        if self.front_obs_dist != -1.0:
            print("----------- front obs dist: ", self.front_obs_dist)
            if ("number_of_wheels" in self.front_actor.attributes.keys()):
                print("### num of wheels: ", self.front_actor.attributes["number_of_wheels"])
            if ("object_type" in self.front_actor.attributes.keys()):
                print("### object type: ", self.front_actor.attributes["object_type"])
            # print("########### front obs attr size: ", len(self.front_actor.attributes))
            # for item in self.front_actor.attributes:
            #     print("###### ", item)
        ...
        ##EXIT CONDITION IF LANE DEPARTURE
        if len(self.crossline_hist) != 0:
            done = True
            exit_program = 1
            reward = -200
            print('There has been a lane departure, distance to target: ', d2target)
            self.summary['Steps'] += 1

        if exit_program == 0:  # IF THERE IS NO EXIT CONDITION FROM THE PROGRAM

            # THE REWARD IS GIVEN BASED ON HOW YOU DO ON THE ROAD
            if reward_mode == 0:
                if kmh < 10:
                    done = False
                    reward = -1
            ...
            # IF YOU HAVE REACHED THE OBJECTIVE, THE REWARD IS CHANGE AND YOU EXIT
            ...
            # IF THE TIMER HAS ENDED THE REWARD IS CHANGE AND YOU EXIT
            if self.episode_start + settings.SECONDS_PER_EPISODE < time.time():
                print('Timer end, distance to target: ', d2target)
                done = True
                self.summary['Steps'] += 1
                if acum <= 50:
                    reward = -200
        ...
        self.cmd_vel = kmh / 120  # normalize the speed

        return reward, done, d2target


    def Calculate_state(self, img2):
        global center_old
        kernel = np.ones((5, 5), np.uint8)
        ...
        for i in range(0, 15):
            ...
            for j in range(0, width):
                if gray[dato_y, j] == 255:
                    waypoint_edges[i][
                        0] = j  # I stick with the x coordinate starting from the left of the image, the y is given by the index
                    break
            for j in range(0, width):
                if gray[dato_y, width - 1 - j] == 255:
                    waypoint_edges[i][
                        1] = width - 1 - j  # I stick with the x coordinate starting from the left of the image, the y is given by the index
                    break
        ...
        if settings.SHOW_WAYPOINTS == 1:
            cv2.namedWindow('Vanishing point', cv2.WINDOW_AUTOSIZE)
            cv2.imshow('Vanishing point', gray)
            cv2.waitKey(1)
        return state
    ...
    def transform2local(self, im):
        ...
        # THE SIZE OF THE WAYPOINT VECTOR IS CHECKING, IF IT IS LESS THAN 15 THE LAST VALUE IS EXTENDED TO THE END.
        ...
        # The normalized value between 100 meters is returned
        if settings.WAYPOINTS == 'XY':
